...
To run Trap Door, use the `honey.py` file. You’ll need to specify a bind IP address (`-a`) and network port (`-p`). Define the protocol type (SSH or HTTP) as well.
...
python3 honey.py -a 0.0.0.0 -p 22 --ssh  
...
```bash
python3 honey.py -a 0.0.0.0 -p 22 --ssh -u admin -pw admin 
```
